...
const state = (): PanelState => ({
  ...
  frames: new Map([[0, { documentId: 'doc-a', atsId: null, url: 'https://example.com/a', fields: [
    { fieldId: 'old', signature: 'old', control: 'text', label: 'Email', required: false },
  ] }], [2, { documentId: 'doc-frame', atsId: null, url: 'https://embedded.example/a', fields: [] }]]),
});
...
it('drops old fields and child frames after a top-frame SPA navigation', () => {
  const next = reduce(state(), { t: 'bg/frameEvent', tabId: 1, frameId: 0, event: { t: 'cs/ready', documentId: 'doc-b', atsId: null, url: 'https://example.com/b' } });
...
it('ignores fields and results from a replaced document even at the same URL', () => {
  const initial = state();
  const next = reduce(initial, { t: 'bg/frameEvent', tabId: 1, frameId: 0, event: { t: 'cs/ready', documentId: 'replacement', atsId: null, url: initial.tabUrl } });
  expect(next.frames.get(0)!.fields).toEqual([]);
  expect(reduce(next, { t: 'bg/frameEvent', tabId: 1, frameId: 0, event: { t: 'cs/fillResults', documentId: 'doc-a', runId: 'old', results: [{fieldId:'old',ok:true}] } })).toBe(next);
});
